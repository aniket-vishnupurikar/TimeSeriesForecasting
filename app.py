...
if option == "Automated Mode":
    ...
    if data is not None:
        ...
        start_ = (complete_data.index[-1] + 1 * complete_data.index.freq).date()
        start = str(start_)
        n_future = 12
        if n_past < 13:
            lags = int(n_past / 10)
        elif n_past < 25 and n_past >= 13:
            lags = int(n_past / 8)
        else:
            lags = int(n_past / 6)
        ...
        file_name = "Forecast_{}.csv".format(time_stamp)
        st.download_button(label="Download Output As CSV",data=csv,file_name=file_name,
                mime='text/csv')
